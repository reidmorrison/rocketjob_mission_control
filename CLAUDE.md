...
1. Remaining pieces of the front-end modernization: the pipeline still uses Sprockets `//= require` manifests (`application.css`/`application.js`) to bundle vendored files, which Propshaft can't process (no directive processor) — a full Propshaft move needs the layout to include each file individually instead. Also Turbo-friendly action links instead of rails-ujs `data-method`. Font Awesome is already on 7.3.0, Selectize has been replaced with tom-select, and the cosmic dark theme (colors, gradients, buttons, nav) was carried through the Bootstrap 5 migration intact rather than needing a separate restoration pass.
