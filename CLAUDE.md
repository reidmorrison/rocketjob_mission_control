...
1. Remaining piece of the front-end modernization: Turbo-friendly action links instead of rails-ujs `data-method`. Font Awesome is already on 7.3.0, Selectize has been replaced with tom-select, the cosmic dark theme (colors, gradients, buttons, nav) was carried through the Bootstrap 5 migration intact rather than needing a separate restoration pass, and the asset pipeline no longer needs Sprockets-only `.css.erb`/`//= require` bundling to be Propshaft-compatible (verified under Sprockets against the dummy app; not yet verified against a real Propshaft host app).
