module RocketjobMissionControl
  class DirmonEntriesController < RocketjobMissionControl::ApplicationController
    ...
    def create
      ...
      if properties = params[:rocketjob_dirmon_entry][:properties]
    ...
    def replicate
      ...
      if dirmon_entry_replicate.errors.empty? && dirmon_entry_replicate.valid? && dirmon_entry_replicate.update_attributes(dirmon_params)
        redirect_to(rocketjob_mission_control.dirmon_entry_path(dirmon_entry_replicate))
      else
        render :copy
      end
    end
    ...
    def update
      ...
      sanitized_params = DirmonSanitizer.sanitize(params[:rocketjob_dirmon_entry], @dirmon_entry.job_class, @dirmon_entry)
      ...
      if @dirmon_entry.errors.empty? && @dirmon_entry.valid? && @dirmon_entry.update_attributes(sanitized_params)
        redirect_to(rocketjob_mission_control.dirmon_entry_path(@dirmon_entry))
      else
        render :edit
      end
    end
    ...
    def dirmon_params
      params.fetch(:rocketjob_dirmon_entry, {}).permit(:name, :archive_directory, :pattern, :job_class_name, :properties)
    end
